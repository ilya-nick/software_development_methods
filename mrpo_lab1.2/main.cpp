...
void AddRecord(const char* path) {
    ...
    cout << "Введите дальность полета:" << endl;
    cin >> airplane->range;
    File.write(reinterpret_cast<char*>(airplane), sizeof(Airplane));
    File.close();
    delete airplane; // Освобождаем память после записи
}
...
void ShowAllRecords(const char* path) {
    ...
    File.close();
    delete airplane; // Освобождаем память после чтения
}
...
void SearchManufacturerAttribute(const char* path, const char* manufacturer) {
    ...
    file.close();
    delete airplane; // Освобождаем память после поиска
...
// Обновленная SearchModelAttribute с сообщением о "Отсутствии данных"
...
// Обновленная SearchYearAttribute с сообщением о "Отсутствии данных"
...
// Обновленная SearchCapacityAttribute с сообщением о "Отсутствии данных"
...
// Обновленная SearchRangeAttribute с сообщением о "Отсутствии данных"
